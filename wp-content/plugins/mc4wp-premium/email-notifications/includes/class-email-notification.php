...
class MC4WP_Email_Notification {
	 ...
	 * @return string
	 */
	private function replace_groupings_tag( $id ) {

		if( empty( $this->data['GROUPINGS'] ) ) {
			return '';
		}
		...
		foreach( $groupings as $grouping ) {
			if( $grouping['id'] == $id ) {
				return $this->readable_value( $grouping['groups'] );
			}
		}
	...
	public function replace_tag( $matches ) {
		...
		if( stripos( $key, 'GROUPINGS' ) === 0 ) {
			return $this->replace_groupings_tag( $subkey );
		}
